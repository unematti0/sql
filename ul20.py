...
def load_data_from_db(tree, search_query=""):
    ...
    # Lisa andmed tabelisse
    for row in rows:
        tree.insert("", "end", values=row[1:], iid=row[0])  # Määrake iid väärtuseks andmebaasi id
...
def open_update_window(record_id):
    ...
    # Loo andmebaasi ühendus ja too olemasolevad andmed
    conn = sqlite3.connect('mattias_elmers.db')
    cursor = conn.cursor()
...
def on_update():
    selected_item = tree.focus()  # Võta valitud rida
    if not selected_item:
        messagebox.showwarning("Valik puudub", "Palun vali kõigepealt rida!")
        return
    record_id = selected_item  # Treeview kasutab iid väärtust, mis on rida ID
    open_update_window(record_id)  # Ava muutmise aken
